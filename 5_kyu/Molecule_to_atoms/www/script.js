...
const getSubFormula = (formula, tree, init) => {
    let i = 0
    ...
    if (beginBracketIndex == null)
        return { parsed_molecule: parseJSON(formula, tree), tree }
    ...
    getSubFormula(right, tree[right])

    if (index.begin != -1) {
        const remainings = remainingFormulas.substring(index['begin'])
        tree[remainings] = {}
        getSubFormula(remainings, tree[remainings])
    }

    return init == null ? 0 : { parsed_molecule: parseJSON(formula, tree), tree }
}
...
const parseRawFormula = (formula, index) => {
    ...
    const parse = {}
    formula.match(/[A-Z]{1}[a-z]{0,3}[0-9]*/g).forEach(molecule => {
        molecule = molecule.match(/([A-Z]{1}[a-z]{0,3})|[0-9]+/g)
